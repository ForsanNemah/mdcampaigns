...
class Arrays
{
	 ...
	 * provided callback with signature `function ($value, $key, $array): bool`.
	 ...
	 * The callback has the signature `function ($value, $key, $array): mixed`.
	 * @template K of array-key
	 * @template V
	 * @template R
	 * @param  array<K, V> $array
	 * @param  callable(V, K, array<K, V>): R $transform
	 * @return array<K, R>
	 */
	public static function map(iterable $array, callable $transform): array
	{
		$res = [];
		foreach ($array as $k => $v) {
			$res[$k] = $transform($v, $k, $array);
		}
